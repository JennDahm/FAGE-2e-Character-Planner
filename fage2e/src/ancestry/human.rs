...
impl std::fmt::Display for AbilityFocusSelection {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::Riding => Focus::DexterityRiding.fmt(f),
            Self::Swimming => Focus::ConstitutionSwimming.fmt(f),
        }
    }
}
...
impl LeafNodeAdvancement for Option<AbilityFocusSelection> {
    fn apply(&self, char: &mut Character) -> Result<bool, ()> {
        let focus = match self {
            ...
            Self::Some(AbilityFocusSelection::Riding) => Focus::DexterityRiding,
            Self::Some(AbilityFocusSelection::Swimming) => Focus::ConstitutionSwimming,
        };
