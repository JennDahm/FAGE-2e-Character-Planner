...
impl std::fmt::Display for AbilityFocusSelection {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::Tolerance => Focus::ConstitutionTolerance.fmt(f),
            Self::Evaluation => Focus::IntelligenceEvaluation.fmt(f),
        }
    }
}
...
impl LeafNodeAdvancement for Option<AbilityFocusSelection> {
    fn apply(&self, char: &mut Character) -> Result<bool, ()> {
        let focus = match self {
            ...
            Self::Some(AbilityFocusSelection::Tolerance) => Focus::ConstitutionTolerance,
            Self::Some(AbilityFocusSelection::Evaluation) => Focus::IntelligenceEvaluation,
        };
